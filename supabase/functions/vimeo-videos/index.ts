...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.error('[VIMEO] Missing Authorization header');
      return new Response(
    ...
    if (authError || !user) {
      console.error('[VIMEO] Invalid authentication:', authError?.message);
      return new Response(
        JSON.stringify({ error: 'Unauthorized' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    console.log('[VIMEO] Authenticated user:', user.id);
