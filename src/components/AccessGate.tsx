...
const subscribeSchema = z.object({
  email: z.string()
    .trim()
    .email("Email inválido / Invalid email")
    .max(255, "Email muito longo / Email too long"),
  full_name: z.string()
    .trim()
    .min(2, "Nome muito curto (mín. 2 caracteres) / Name too short (min 2 chars)")
    .max(100, "Nome muito longo (máx. 100 caracteres) / Name too long (max 100 chars)")
});
...
export const AccessGate = ({ onAccessGranted }: AccessGateProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate input
    const validation = subscribeSchema.safeParse({ email, full_name: fullName });
    ...
    try {
      ...
      toast.success(
        "Bem-vindo ao THEVOIDN13! Verifique seu email para confirmar o cadastro. / " +
        "Welcome to THEVOIDN13! Check your email to confirm registration."
      );
      ...
      toast.error(
        error.message || 
        "Erro ao processar cadastro / Error processing registration"
      );
    } finally {
